...
def build_pretrained_inception_v3():

    net = build_network()  # net is a dict of layers (layers are already linked)
    weightfile = path.join(path.dirname(__file__), 'inception_v3.pkl')
    ...
    set_all_param_values(net['softmax'], thetas)  # net.prob is the last layer, set_all works on all downstream layers
